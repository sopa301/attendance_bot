...
from util.helper import compare_time, parse_dt_to_iso
from util.objects import AttendanceList, EventPoll, PollGroup, Status
from util.texts import (
    ATTENDANCE_MENU_TEXT,
    CANCEL_TEXT,
...
async def create_new_poll(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "What would you like to call this poll?\n" + POLL_GROUP_TEMPLATE
    )
...
async def get_poll_name(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please input the number of events you want to poll for."
    )
...
async def get_number_of_events(update: Update, context: CustomContext) -> int:
    try:
        ...
        await update.message.reply_text(
            f"Please input the details of event 1.\n" + DETAILS_TEMPLATE
        )
        return routes["GET_DETAILS"]
    except ValueError as e:
        await update.message.reply_text(
            "Please input a valid number that is at least 1."
        )
        return routes["GET_NUMBER_OF_EVENTS"]
...
async def get_details(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please input the start time of the event.\n" + DATE_FORMAT_TEMPLATE
    )
...
async def get_start_time(update: Update, context: CustomContext) -> int:
    ...
    if not status.status:
        await update.message.reply_text(
            f"Unsuccessful: {status.message}. Please input start time again"
        )
        return routes["GET_START_TIME"]

    context.user_data["start_time"] = start_dt
    await update.message.reply_text(
        "Please input the end time of the event.\n" + DATE_FORMAT_TEMPLATE
    )
...
async def get_end_time(update: Update, context: CustomContext) -> int:
    ...
    if not status.status:
        await update.message.reply_text(
            f"Unsuccessful: {status.message}. Please input end time again"
        )
        return routes["GET_END_TIME"]

    if compare_time(context.user_data["start_time"], et) > 0:
        await update.message.reply_text(
            f"Unsuccessful: end time given is before start time. Please input end time again"
        )
        return routes["GET_END_TIME"]
    ...
    poll = EventPoll(st, et, details, [100, 100])
    ...
    if current_event_count <= total_events:
        await update.message.reply_text(
            f"Please input the details of event {current_event_count}."
        )
        return routes["GET_DETAILS"]
...
def set_person_in_event(
    poll_id, username: str, membership: Membership, is_sign_up: bool
) -> None:
...
async def handle_poll_voting_callback(update: Update, context: CustomContext) -> None:
    ...
    if user.username is None:
        await update.callback_query.answer(
            text="Please set a username in your Telegram settings to vote.",
            show_alert=True,
        )
        return
    ...
    if not is_changed:
        ...
        await update.callback_query.answer(
            text=f"You have been {new_status} for {poll.get_title()}.", show_alert=True
        )
        return
...
async def handle_generate_next_poll_callback(
    ...
    await update.callback_query.edit_message_text(
        "What would you like to name the new poll?\n" + POLL_GROUP_TEMPLATE
    )
    ...
    for poll in polls:
        ...
        for membership in Membership:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"{membership.to_representation()}{poll.get_active_status_representation(membership)}",
...
async def handle_manage_active_poll_callback(
    update: Update, context: CustomContext
) -> None:
    user = update.callback_query.from_user
    ...
    await update.callback_query.edit_message_text(
        "You can activate/deactivate polls here. Note that it also affects polls already published.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
    return ConversationHandler.END
...
async def attendance(update: Update, context: CustomContext) -> int:
    ...
    logger.info(
        "User %s requested to manage attendance.", update.message.from_user.first_name
    )
...
conv_handler = ConversationHandler(
    entry_points=[
        ...
        CommandHandler("info", get_info),
        CommandHandler("polls", get_polls),
        CallbackQueryHandler(
            handle_generate_next_poll_callback, pattern=GENERATE_NEXT_POLL_REGEX_STRING
        ),
    ],
    states={
        routes["GET_NUMBER_OF_EVENTS"]: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, get_number_of_events)
        ],
        routes["GET_DETAILS"]: [
        ...
        routes["GET_POLL_NAME"]: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, get_poll_name)
        ],
        routes["GET_NEW_POLL_NAME"]: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, create_next_poll)
        ],
    },
...
attendance_conv_handler = ConversationHandler(
    entry_points=[
        ...
        CommandHandler("attendance_tracking", attendance_tracking),
    ],
    states={
        routes["SELECT_NEW_OR_CONTINUE"]: [
            CommandHandler("new_list", request_attendance_list),
            CommandHandler("view_lists", get_lists),
            CommandHandler("import_from_poll", import_from_poll),
        ],
        routes["MANAGE_ATTENDANCE_LIST"]: [
            CallbackQueryHandler(
                handle_manage_attendance_list,
                pattern=MANAGE_ATTENDANCE_LIST_REGEX_STRING,
            )
        ],
        routes["RECEIVE_INPUT_LIST"]: [
            MessageHandler(
                filters.TEXT & ~filters.COMMAND, process_inputted_attendance_list
            )
        ],
        routes["VIEW_LIST"]: [
            CallbackQueryHandler(
                handle_view_attendance_list, pattern=VIEW_ATTENDANCE_LISTS_REGEX_STRING
            )
        ],
        routes["INPUT_LIST"]: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, request_attendance_list)
        ],
        routes["RECEIVE_EDITED_LIST"]: [
            MessageHandler(
                filters.TEXT & ~filters.COMMAND, process_edited_attendance_list
            )
        ],
...
application.add_handler(
    CallbackQueryHandler(handle_poll_voting_callback, pattern=POLL_VOTING_REGEX_STRING)
)
application.add_handler(
    CallbackQueryHandler(
        handle_update_results_callback, pattern=UPDATE_POLL_RESULTS_REGEX_STRING
    )
)
application.add_handler(
    CallbackQueryHandler(
        handle_manage_active_poll_callback, pattern=MANAGE_ACTIVE_POLLS_REGEX_STRING
    )
)
application.add_handler(
    CallbackQueryHandler(
        handle_change_poll_active_status_callback,
        pattern=SET_POLL_ACTIVE_STATUS_REGEX_STRING,
    )
)
application.add_handler(
    CallbackQueryHandler(handle_delete_poll_callback, pattern=DELETE_POLL_REGEX_STRING)
)
application.add_handler(
    CallbackQueryHandler(
        poll_title_clicked_callback, pattern=MANAGE_POLL_GROUPS_REGEX_STRING
    )
)
application.add_handler(
    CallbackQueryHandler(
        handle_view_attendance_summary, pattern=VIEW_SUMMARY_REGEX_STRING
    )
)
application.add_handler(
    CallbackQueryHandler(change_status, pattern=MARK_ATTENDANCE_REGEX_STRING)
)
application.add_handler(
    CallbackQueryHandler(do_nothing, pattern=DO_NOTHING_REGEX_STRING)
)
application.add_handler(
    CallbackQueryHandler(
        handle_view_attendance_tracking,
