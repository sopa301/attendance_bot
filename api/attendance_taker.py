...
def generate_inline_keyboard_for_attendance_tracking_format(
    attendance_lists: list,
) -> list:
    inlinekeyboard = []
    for attendance_list in attendance_lists:
        inlinekeyboard.append(
            [
                InlineKeyboardButton(
                    attendance_list.get_title(),
                    callback_data=encode_view_attendance_tracking_format(
                        attendance_list.id
                    ),
                )
            ]
...
async def get_lists(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please select the attendance list you want to edit.",
...
async def import_from_poll(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please select the poll group you want to import from.",
...
async def handle_select_poll_group(update: Update, context: CustomContext) -> int:
    ...
    await update.callback_query.edit_message_text(
        "Please select the poll you want to import from.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def handle_view_attendance_list(update: Update, context: CustomContext) -> int:
    ...
    await update.callback_query.edit_message_text(
        f"Currently managing {attendance_list.get_title()}. What would you like to do?",
        reply_markup=InlineKeyboardMarkup(inlinekeyboard),
    )
...
async def handle_manage_attendance_list(update: Update, context: CustomContext) -> int:
    ...
    if command == "take_attendance":
        ...
        await update.callback_query.edit_message_text(
            "Please copy and edit the list, then send it to me."
        )
        await update.callback_query.message.reply_text(
            attendance_list.to_parsable_list()
        )
        ...
        delete_attendance_list(attendance_list.id)
        await update.callback_query.edit_message_text(
            "Attendance list logged and deleted."
        )
        return ConversationHandler.END
    else:
...
async def process_edited_attendance_list(update: Update, context: CustomContext) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            "Invalid list format. Please input the list again."
        )
        logger.info(e)
        return routes["RECEIVE_EDITED_LIST"]
...
async def summary(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please select the attendance list you want to view the summary of.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
    return ConversationHandler.END
...
async def attendance_tracking(update: Update, context: CustomContext) -> int:
    ...
    await update.message.reply_text(
        "Please select the attendance list you want to view the attendance tracking format of.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
    return ConversationHandler.END


async def handle_view_attendance_tracking(
    update: Update, context: CustomContext
) -> int:
...
async def change_status(update: Update, context: CustomContext) -> None:
    ...
    logger.info(
        "User %s selected %s with id %s, set to %s",
        user.first_name,
        selected_user.name,
        selected_user.id,
